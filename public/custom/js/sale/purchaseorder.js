"use strict";
    ...
    function addRowToInvoiceItemsTable(recordObject, loadedFromUpdateOperation=false){
       //Find service table row count
        ...
        var inputQuantity = '<input required type="number" name="quantity[' + currentRowId + ']" class="form-control" value="" oninput="checkstatus(this.value, \'' + recordObject.available_stock + '\', \'' + currentRowId + '\')">';


        var current_stock   = '<input type="hidden" name="current_stock['+ currentRowId +']" class="form-control" value="' + recordObject.current_stock + '">'+recordObject.current_stock;
         var available_stock   = '<input type="hidden" name="available_stock['+ currentRowId +']" class="form-control" value="' + recordObject.available_stock + '">'+recordObject.available_stock;
        var current_status   = '<input type="hidden" name="status['+ currentRowId +']" class="form-control" id="curstatus-'+currentRowId+'"><span id="curstatustext-'+currentRowId+'"></span>';
        ...
        var remarks  = '<textarea rows="1" type="text" name="remarks['+ currentRowId +']" id="remarks-'+ currentRowId +'" class="form-control" placeholder="Remarks"></textarea>';

        var pakingremarks  = '<textarea rows="1" type="text" name="pakingremarks['+ currentRowId +']" id="pakingremarks-'+ currentRowId +'" class="form-control" placeholder="Packing Remarks"></textarea>';

        var dispatchremarks  = '<textarea rows="1" type="text" name="dispatchremarks['+ currentRowId +']" id="dispatchremarks-'+ currentRowId +'" class="form-control" placeholder="Dispatch Remarks"></textarea>';
        ...
        var inputDeleteButton = '<button type="button" class="btn btn-outline-danger '+removeClass+'"><i class="bx bx-trash me-0"></i></button>';
         var brand = `<select name="brand_id[${currentRowId}]" class="form-control brand-select" id="brand-select-${currentRowId}">
                    <option value="">Loading brands...</option>
                </select>`;
          var category =  `<select name="category_id[${currentRowId}]" class="form-control category-select" id="category-select-${currentRowId}">
        ...
        var newRow = $('<tr id="'+ currentRowId +'" class="highlight">');
            ...
            newRow.append('<td>' + current_stock + '</td>');
            newRow.append('<td>' + available_stock + '</td>');
            newRow.append('<td>' + inputQuantity + '</td>');
            newRow.append('<td>' + brand + '</td>');
            newRow.append('<td>' + category + '</td>');           
            newRow.append('<td>'+current_status+'</td>');
            newRow.append('<td>' + remarks + '</td>');
            newRow.append('<td>' + pakingremarks + '</td>');
            newRow.append('<td>' + dispatchremarks + '</td>');
   ...
   function loadBrandsForRow(rowId) {
        $.ajax({
            ...
            success: function(response) {
                ...
                // Add brand options from the response
            ...
            error: function(xhr, status, error) {
                ...
                $('#brand-select-'+rowId).html('<option value="">Error loading brands</option>');
            }
        });
    }
  ...
  function loadCategoryForRow(rowId) {
        $.ajax({
            ...
            dataType: 'json',
            success: function(response) {
                var select = $('#category-select-'+rowId);
                select.empty();
   ...
   /**
    ...
    function setBottomOfTableRecords(){
        ...
        updateSubmitButtonStatus();        //check all items status for display button(product or dispatch)
    ...
    * */
    ...
    itemSearchInputBoxId.autocomplete({
        ...
        open: function(event, ui) {
                    ...
                    var header = $("<li class='ui-autocomplete-category' style='padding: 5px; border-bottom: 1px solid #ddd; background-color: #f8f9fa;'>" +
                        ...
                        "<span style='flex: 0;'>Brand</span>" +
                        "<span style='flex: 1; text-align: right;'>Current Stock</span>" +
                        "<span style='flex: 1; text-align: right;'>Available Stock</span>" +
                        "</div></li>");
        ...
        return $("<li>")
            ...
            .append(`<div style="display: flex; align-items: center;">
                        <span style="flex: 2; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${item.name || 'N/A'}</span>
                        <span style="flex: 0; text-align: left;">${item.brand_name}</span>
                        ...
                        <span style="flex: 1; text-align: right; color: ${_parseQuantity(item.available_stock) > 0 ? '#000000' : '#dc3545'};">${_parseQuantity(item.available_stock) || 'N/A'}</span>
                     </div>`)
            .appendTo(ul);
    };
    ...
    $(document).ready(function(){
        ...
        $(".dispatch-class").addClass("d-none");
        /**
   ...
   function checkstatus(quantity, stock, rowid) {
        quantity = parseInt(quantity);
        stock = parseInt(stock);
        var statusValueSelector = '#curstatus-' + rowid;
        var statusTextSelector = '#curstatustext-' + rowid;
        $('#remarks-' + rowid).prop('disabled',false);
        $(statusTextSelector).removeClass("badge rounded-pill text-success text-danger bg-light-success p-2 text-uppercase px-3");
        $(statusValueSelector).val("");
        $(statusTextSelector).text("");
        if (quantity <= stock) {
            $(statusValueSelector).val("Ready to Dispatch");
            $(statusTextSelector).text("Ready to Dispatch");
            $(statusTextSelector).addClass("badge rounded-pill text-success bg-light-success p-2 text-uppercase px-3");
        } else {
            $(statusValueSelector).val("Push To Production");
            $(statusTextSelector).text("Push To Production");
            // $('#remarks-' + rowid).prop('disabled',false);
            $(statusTextSelector).addClass("badge rounded-pill text-danger bg-light-success p-2 text-uppercase px-3");
        }
    ...
    function updateSubmitButtonStatus() {
        ...
        $('input[id^="curstatus-"]').each(function () {
            if ($(this).val() !== 'Ready to Dispatch') {
                allReady = false;
        ...
        if (allReady) {
            $('#submit_form').text("Ready to Dispatch");
            $(".dispatch-class").removeClass("d-none");
        } else {
            $('#submit_form').text("Push To Production");
        }
    }
